...
- 레이아웃: 상단 고정 헤더(로고 + 검색 + 계정), 우측 사이드바(최근 변경/랜덤 문서/도구), 본문 우측 상단에 편집·역사·비교·역링크 버튼 그룹(pill 스타일 + 아이콘).
...
- [x] **P0** 프로젝트 셋업, Supabase 스키마, 문서 보기/편집/저장, 역사, diff
- [x] **P1** 위키 문법 파서 1~6단계, 검색, 최근 변경, 분류·역링크
- [x] **P2** 파서 7~10단계(완료: 블록 {{{ }}}, 각주, 매크로 br/목차/date/age/dday/ruby/anchor/pagecount/include, #redirect 콘텐츠 동기화), 문서 이동·삭제·되돌리기(완료) — 남은 건 파일 업로드뿐
- [x] **P3** 토론 시스템(스레드 생성·댓글·상태 변경), 편집 요청(제출·목록), 신고(문서·토론 댓글 접수) — 검토·승인/거절·처리 큐는 관리자 도구와 함께 P4에서
